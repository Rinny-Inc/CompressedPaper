package net.badlion.gspigot.protocol210;
...
public class MetadataWriter {
	public static void write(PacketDataSerializer serializer, Object[] data, Class<? extends Entity> clss) throws IOException {
		...
		if (EntityLiving.class.isAssignableFrom(clss)) {
			writeFloat(serializer, 7, (Float) data[6]);
			writeInt(serializer, 8, (Integer) data[7]);
			writeBooleanFromByte(serializer, 9, (Byte) data[8]);
			writeInt(serializer, 10, (Byte) data[9]);
			...
			if (EntityAgeable.class.isAssignableFrom(clss)) {
				if (data[12] instanceof Integer)
					writeBoolean(serializer, 12, (((Integer) data[12]).intValue() < 0));
				if (EntityTameableAnimal.class.isAssignableFrom(clss)) {
					writeByte(serializer, 13, (Byte) data[16]);
					if (EntityOcelot.class.isAssignableFrom(clss)) {
						writeInt(serializer, 15, (Byte) data[18]);
					} else if (EntityWolf.class.isAssignableFrom(clss)) {
						writeFloat(serializer, 15, (Float) data[18]);
						writeBooleanFromByte(serializer, 15, (Byte) data[19]);
						writeInt(serializer, 17, (Byte) data[20]);
					}
				} else if (EntityPig.class.isAssignableFrom(clss)) {
					writeBooleanFromByte(serializer, 13, (Byte) data[16]);
				} else if (EntitySheep.class.isAssignableFrom(clss)) {
					writeByte(serializer, 13, (Byte) data[16]);
				} else if (EntityVillager.class.isAssignableFrom(clss)) {
					writeInt(serializer, 13, (Integer) data[16]);
				} else if (EntityHorse.class.isAssignableFrom(clss)) {
					writeByte(serializer, 13, (Integer) data[16]);
					writeInt(serializer, 14, (Byte) data[19]);
					writeInt(serializer, 15, (Integer) data[20]);
					writeInt(serializer, 17, (Integer) data[22]);
				}
			} else if (EntityHuman.class.isAssignableFrom(clss)) {
				writeByte(serializer, 13, (Byte) data[16]);
				writeFloat(serializer, 11, (Float) data[17]);
				...
				if (data[16] instanceof Byte || data[17] instanceof Byte) {
					...
					writeBlockId(serializer, 12, blockId, blockData);
				}
				writeBooleanFromByte(serializer, 13, (Byte) data[18]);
			} else if (EntityZombie.class.isAssignableFrom(clss)) {
				writeBooleanFromByte(serializer, 12, (Byte) data[12]);
				writeInt(serializer, 13, (Byte) data[13]);
				writeBooleanFromByte(serializer, 14, (Byte) data[14]);
				...
				writeInt(serializer, 12, (Byte) data[16]);
				writeBooleanFromByte(serializer, 13, (Byte) data[17]);
				...
				writeBooleanFromByte(serializer, 12, (Byte) data[21]);
			} else if (EntityIronGolem.class.isAssignableFrom(clss)) {
				writeByte(serializer, 12, (Byte) data[16]);
			} else if (EntityWither.class.isAssignableFrom(clss)) {
				writeInt(serializer, 12, (Integer) data[17]);
				writeInt(serializer, 13, (Integer) data[18]);
				writeInt(serializer, 14, (Integer) data[19]);
				writeInt(serializer, 15, (Integer) data[20]);
			}
		} else if (EntityBoat.class.isAssignableFrom(clss)) {
			writeInt(serializer, 6, (Integer) data[17]);
			writeInt(serializer, 7, (Integer) data[18]);
			writeFloat(serializer, 8, (Float) data[19]);
		} else if (EntityMinecartAbstract.class.isAssignableFrom(clss)) {
			writeInt(serializer, 6, (Integer) data[17]);
			writeInt(serializer, 7, (Integer) data[18]);
			writeFloat(serializer, 8, (Float) data[19]);
			if (data[20] instanceof Integer) {
				int blockId = ((Integer) data[20]).intValue() & 0xFFFF;
				int blockData = ((Integer) data[20]).intValue() >> 16;
				writeInt(serializer, 9, Integer.valueOf(blockId | blockData << 12));
			}
			writeInt(serializer, 10, (Integer) data[21]);
			writeBooleanFromByte(serializer, 11, (Byte) data[22]);
			if (EntityMinecartFurnace.class.isAssignableFrom(clss))
				writeBooleanFromByte(serializer, 12, (Byte) data[16]);
		} else if (EntityItem.class.isAssignableFrom(clss)) {
			writeItemStack(serializer, 6, data[10]);
		} else if (EntityItemFrame.class.isAssignableFrom(clss)) {
			writeItemStack(serializer, 6, data[2]);
			if (data[3] instanceof Byte)
				writeInt(serializer, 7, Integer.valueOf(((Byte) data[3]).byteValue() * 2));
		} else if (EntityArrow.class.isAssignableFrom(clss)) {
			writeByte(serializer, 6, (Byte) data[16]);
		} else if (EntityFireworks.class.isAssignableFrom(clss)) {
			writeItemStack(serializer, 6, data[8]);
		} else if (EntityPotion.class.isAssignableFrom(clss)) {
			writeItemStack(serializer, 7, data[6]);
		}
	...
	private static void writeItemStack(PacketDataSerializer serializer, int index, Object itemStack) {
		if (itemStack instanceof ItemStack) {
			...
			serializer.a((ItemStack) itemStack);
