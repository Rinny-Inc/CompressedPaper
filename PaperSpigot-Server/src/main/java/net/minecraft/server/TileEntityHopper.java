...
public class TileEntityHopper extends TileEntity implements IHopper {
    ...
    public void convertToPolling() {
        long cooldownDiff = this.nextTick - this.world.getTime() + (this.lastTick == this.world.getTime() ? 0 : 1);
    ...
    public static EntityItem getEntityItemAt(World world, double d0, double d1, double d2) {
        List<Entity> list = world.a(EntityItem.class, AxisAlignedBB.a(d0, d1, d2, d0 + 1.0D, d1 + 1.0D, d2 + 1.0D), IEntitySelector.a);

        return !list.isEmpty() ? (EntityItem) list.get(0) : null; // Rinny - use !isEmpty instead of size > 0
    } 
    ...
    public static IInventory getInventoryAt(World world, double d0, double d1, double d2) {
        ...
        if (iinventory == null) {
            List<Entity> list = world.getEntities((Entity) null, AxisAlignedBB.a(d0, d1, d2, d0 + 1.0D, d1 + 1.0D, d2 + 1.0D), IEntitySelector.c);

            if (list != null && !list.isEmpty()) { // Rinny - use !isEmpty instead of size > 0
