...
public abstract class World implements IBlockAccess {
    ...
    // Spigot end
    protected List<Entity> f = new ArrayList<Entity>();
    public Set<TileEntity> tileEntityList = new org.spigotmc.WorldTileEntityList(this); // CraftBukkit - ArrayList -> HashSet
    private List<TileEntity> a = new ArrayList<TileEntity>();
    private List<TileEntity> b = new ArrayList<TileEntity>();
    public List<EntityHuman> players = new ArrayList<>();
    public List<Entity> i = new ArrayList<Entity>();
    ...
    // CraftBukkit end
    private ArrayList<AxisAlignedBB> L;
    private boolean M;
    int[] I;
    ...
    public void updateChestAndHoppers(int a, int b, int c) {
    	final Block block = this.getType(a, b, c);
    ...
    public World(IDataManager idatamanager, String s, WorldSettings worldsettings, WorldProvider worldprovider, ChunkGenerator gen, org.bukkit.World.Environment env) {
        ...
        final PersistentVillage persistentvillage = (PersistentVillage) this.worldMaps.get(PersistentVillage.class, "villages");
    ...
    // Spigot end
        	final Iterator<BlockState> it = capturedBlockStates.iterator();
            while (it.hasNext()) {
        ...
        // CraftBukkit end
        final int x = 30000000;
        if (i >= -x && k >= -x && i < x && k < x && j >= 0 && j < 256) {
            Chunk chunk = null;
    ...
    public boolean setTypeAndData(int i, int j, int k, Block block, int l, int i1) {
        ...
        if (i >= -x && k >= -x && i < x && k < x) {
            if (j < 0) {
                ...
                final boolean flag = chunk.a(i & 15, j, k & 15, block, l);
    ...
    public boolean setData(int i, int j, int k, int l, int i1) {
        ...
        if (i >= -30000000 && k >= -30000000 && i < 30000000 && k < 30000000) {
            if (j < 0) {
            	...
            	final int j1 = i & 15;
            	final int k1 = k & 15;
            	final boolean flag = chunk.a(j1, j, k1, l);

                if (flag) {
                	final Block block = chunk.getType(j1, j, k1);
    ...
    public boolean setAir(int i, int j, int k, boolean flag) {
        ...
        if (block.getMaterial() == Material.AIR) {
        	...
        	final int l = this.getData(i, j, k);
    ...
    public void b(int i, int j, int k, int l) {
        ...
        if (!this.worldProvider.g) {
        	final Chunk chunk = getChunkIfLoaded(i >> 4, j >> 4); // PandaSpigot
        	for (i1 = k; chunk != null && i1 <= l; ++i1) { // PandaSpigot
        		this.updateBrightness(EnumSkyBlock.SKY, i, i1, j, chunk); // PaperSpigot - Asynchronous lighting updates // PandaSpigot - pass chunk so we don't have to find it again while it's known
        	}
            /*for (i1 = k; i1 <= l; ++i1) {
    ...
    public void applyPhysics(int i, int j, int k, Block block) {
    	...
    	for (byte[] direction : DIRECTIONS) {
            ...
            int newZ = k + direction[2];
            this.e(newX, newY, newZ, block);
        }
    ...
    public void b(int x, int y, int z, Block block, int l) {
    	for (int dir = 0; dir < DIRECTIONS.length; dir++) {
            if (dir != l) {
                int newX = x + DIRECTIONS[dir][0];
                int newY = y + DIRECTIONS[dir][1];
                int newZ = z + DIRECTIONS[dir][2];
                this.e(newX, newY, newZ, block);
            }
        }
        /*if (l != 4) {
            this.e(x - 1, y, z, block);
        }
    ...
    public int b(int i, int j, int k, boolean flag) {
        if (i >= -30000000 && k >= -30000000 && i < 30000000 && k < 30000000) {
            if (flag && this.getType(i, j, k).n()) {
                ...
                final int i1 = this.b(i + 1, j, k, false);
                final int j1 = this.b(i - 1, j, k, false);
                final int k1 = this.b(i, j, k + 1, false);
                final int l1 = this.b(i, j, k - 1, false);
    ...
    public int b(EnumSkyBlock enumskyblock, int i, int j, int k) {
        ...
        if (i >= -30000000 && k >= -30000000 && i < 30000000 && k < 30000000) {
            ...
            final Chunk chunk = this.getChunkAt(l, i1);
        ...
        if (!Double.isNaN(vec3d.a) && !Double.isNaN(vec3d.b) && !Double.isNaN(vec3d.c)) {
            if (!Double.isNaN(vec3d1.a) && !Double.isNaN(vec3d1.b) && !Double.isNaN(vec3d1.c)) {
                ...
                if ((!flag1 || block.a(this, l, i1, j1) != null) && block.a(k1, flag)) {
                	final MovingObjectPosition movingobjectposition = block.a(this, l, i1, j1, vec3d, vec3d1);
                ...
                while (k1-- >= 0) {
                    ...
                    final boolean flag3 = !(i == l);
                    final boolean flag4 = !(j == i1);
                    final boolean flag5 = !(k == j1);
                    ...
                    final double d3 = !flag3 ? 999.0D : (d0 - vec3d.a) / d6;
                    final double d4 = !flag4 ? 999.0D : (d1 - vec3d.b) / d7;
                    final double d5 = !flag5 ? 999.0D : (d2 - vec3d.c) / d8;
                    ...
                    if (!flag1 || block1.a(this, l, i1, j1) != null) {
                        if (block1.a(l1, flag)) {
                            ...
                            movingobjectposition = new MovingObjectPosition(l, i1, j1, b0, vec3d, false);
                        }
                    }
                }
                return flag2 ? movingobjectposition : null;
    ...
    public void makeSound(Entity entity, double d0, double d1, double d2, String s, float f, float f1) {
    	for (int i = 0; i < this.u.size(); i++) {
    		if (entity instanceof EntityHuman) {
    			((IWorldAccess)this.u.get(i)).a((EntityHuman)entity, s, d0, d1, d2, f, f1);
    			continue;
    		}
    		((IWorldAccess)this.u.get(i)).a(s, d0, d1 - entity.height, d2, f, f1);
    	}
    }
    ...
    public boolean addEntity(Entity entity, SpawnReason spawnReason) { // Changed signature, added SpawnReason
        ...
        //Bukkit.broadcastMessage("spawned " + entity.getName() + " " + spawnReason.name() + " #" + entity.getId());
        ...
        // Spigot start
        else if (entity instanceof EntityExperienceOrb xp) { // Rinny
        	final double radius = spigotConfig.expMerge;
            if (radius > 0) {
            	final List<Entity> entities = this.getEntities(entity, entity.boundingBox.grow(radius, radius, radius));
                for (Entity e : entities) {
                    if (e instanceof EntityExperienceOrb loopItem) { // Rinny
                        if (!loopItem.dead) {
    ...
    public List<AxisAlignedBB> getCubes(Entity entity, AxisAlignedBB axisalignedbb) {
        ...
        final int j = MathHelper.floor(axisalignedbb.d + 1.0D);
        final int k = MathHelper.floor(axisalignedbb.b);
        final int l = MathHelper.floor(axisalignedbb.e + 1.0D);
        final int i1 = MathHelper.floor(axisalignedbb.c);
        final int j1 = MathHelper.floor(axisalignedbb.f + 1.0D);
        ...
        for ( int chunkx = ( i >> 4 ); chunkx <= ( ( j - 1 ) >> 4 ); chunkx++ )
        {
            ...
            for ( int chunkz = ( i1 >> 4 ); chunkz <= ( ( j1 - 1 ) >> 4 ); chunkz++ )
            {
                ...
                final int zstart = ( i1 < cz ) ? cz : i1;
                final int zend = ( j1 < ( cz + 16 ) ) ? j1 : ( cz + 16 );
                ...
                for ( int x = xstart; x < xend; x++ )
                {
                    for ( int z = zstart; z < zend; z++ )
                    {
                        ...
                        {
                        	final Block block = chunk.getType(x - cx, y, z - cz );
                            if ( block != null )
                            {
                                ...
                                if (entity.world.paperSpigotConfig.fallingBlocksCollideWithSigns && (entity instanceof EntityTNTPrimed || entity instanceof EntityFallingBlock) &&
                                	...
                                	final AxisAlignedBB aabb = AxisAlignedBB.a(x, y, z, x + 1.0, y + 1.0, z + 1.0);
        ...
        final List<Entity> list = this.getEntities(entity, axisalignedbb.grow(d0, d0, d0));
            ...
            for (int l1 = i1; l1 < j1; ++l1) {
                if (this.isLoaded(k1, 64, l1)) {
                    for (int i2 = k - 1; i2 < l; ++i2) {
                    	final Block block = (k1 >= -30000000 && k1 < 30000000 && l1 >= -30000000 && l1 < 30000000 ? this.getType(k1, i2, l1) : Blocks.BEDROCK);
    ...
    public int i(int i, int j) {
        ...
        for (j &= 15; k > 0; --k) {
        	final Block block = chunk.getType(i, k, j);
        ...
        if (!this.a.isEmpty()) {
            for (TileEntity tileentity : this.a) {
                if (!tileentity.r()) {
                    ...
                    if (this.isChunkLoaded(tileentity.x >> 4, tileentity.z >> 4)) {
                    	final Chunk chunk1 = this.getChunkAt(tileentity.x >> 4, tileentity.z >> 4);

                        if (chunk1 != null) {
                            chunk1.a(tileentity.x & 15, tileentity.y, tileentity.z & 15, tileentity);
    ...
    public void entityJoinedWorld(Entity entity, boolean flag) {
        ...
        if (!org.spigotmc.ActivationRange.checkIfActive(entity)) {
            ...
            final int k = MathHelper.floor(entity.locX / 16.0D);
            //int l = MathHelper.floor(entity.locY / 16.0D);
            final int l = Math.min(15, Math.max(0, MathHelper.floor(entity.locY / 16.0D))); // Paper - stay consistent with chunk add/remove behavior
            final int i1 = MathHelper.floor(entity.locZ / 16.0D);
    ...
    public boolean a(AxisAlignedBB axisalignedbb, Entity entity) { // TODO ???
    	final List<Entity> list = this.getEntities((Entity) null, axisalignedbb);
        ...
        for (int k1 = i; k1 < j; ++k1) {
            for (int l1 = k; l1 < l; ++l1) {
                for (int i2 = i1; i2 < j1; ++i2) {
                	final Block block = this.getType(k1, l1, i2);
                ...
                for (int l1 = k; l1 < l; ++l1) {
                    for (int i2 = i1; i2 < j1; ++i2) {
                        ...
                        if (block.getMaterial() == material) {
                        	final double d0 = (double) ((float) (l1 + 1) - BlockFluids.b(this.getData(k1, l1, i2)));
        ...
        for (int k1 = i; k1 < j; ++k1) {
            for (int l1 = k; l1 < l; ++l1) {
                for (int i2 = i1; i2 < j1; ++i2) {
                    ...
                    if (block.getMaterial() == material) {
                    	final int j2 = this.getData(k1, l1, i2);
    ...
    public float a(Vec3D vec3d, AxisAlignedBB axisalignedbb) {
    	final double d0 = 1.0D / ((axisalignedbb.d - axisalignedbb.a) * 2.0D + 1.0D);
    	final double d1 = 1.0D / ((axisalignedbb.e - axisalignedbb.b) * 2.0D + 1.0D);
    	final double d2 = 1.0D / ((axisalignedbb.f - axisalignedbb.c) * 2.0D + 1.0D);
    	...
    	final double xOffset = (1.0 - Math.floor(1.0 / d0) * d0) / 2.0;
    	final double zOffset = (1.0 - Math.floor(1.0 / d2) * d2) / 2.0;
        // PaperSpigot end
        ...
        if (d0 >= 0.0D && d1 >= 0.0D && d2 >= 0.0D) {
            ...
            final Vec3D vec3d2 = vec3d.a(0, 0, 0); // CraftBukkit
            for (float f = 0.0F; f <= 1.0F; f = (float) ((double) f + d0)) {
                for (float f1 = 0.0F; f1 <= 1.0F; f1 = (float) ((double) f1 + d1)) {
                    for (float f2 = 0.0F; f2 <= 1.0F; f2 = (float) ((double) f2 + d2)) {
                    	final double d3 = axisalignedbb.a + (axisalignedbb.d - axisalignedbb.a) * (double) f;
                    	final double d4 = axisalignedbb.b + (axisalignedbb.e - axisalignedbb.b) * (double) f1;
                    	final double d5 = axisalignedbb.c + (axisalignedbb.f - axisalignedbb.c) * (double) f2;
    ...
    public TileEntity getTileEntity(int i, int j, int k) {
        if (j >= 0 && j < 256) {
            ...
            if (this.M) {
                for (TileEntity tileentity1 : this.a) {
    ...
    protected void o() {
        if (!this.worldProvider.g) {
            if (!this.isStatic) {
                ...
                if (j <= 0) {
                    this.worldData.setWeatherDuration(this.random.nextInt(this.worldData.hasStorm() ? 12000 : 168000) + 12000);
                } else {
                    --j;
    ...
    protected void C() {
        ...
        */
        ...
        // TODO: check if it does shit (if it does add + 0.5 in the Math.ceil)
        ...
        for (EntityHuman entityhuman : this.players) {
            ...
            final int existingPlayers = Math.max( 0, chunkTickList.get( key ) ); // filter out -1
            chunkTickList.put(key, (short) (existingPlayers + 1));

            // Check and see if we update the chunks surrounding the player this tick
            for ( int chunk = 0; chunk < chunksPerPlayer; chunk++ )
            {
            	final int dx = ( random.nextBoolean() ? 1 : -1 ) * random.nextInt( randRange );
            	final int dz = ( random.nextBoolean() ? 1 : -1 ) * random.nextInt( randRange );
    ...
    protected void a(int i, int j, Chunk chunk) {
        if (!this.paperSpigotConfig.disableMoodSounds && this.K == 0 && !this.isStatic) { // PaperSpigot - Disable mood sounds
            ...
            int l = k & 15;
            int i1 = k >> 8 & 15;
            final int j1 = k >> 16 & 255;
            final Block block = chunk.getType(l, j1, i1);

            l += i;
            i1 += j;
            if (block.getMaterial() == Material.AIR && this.j(l, j1, i1) <= this.random.nextInt(8) && this.b(EnumSkyBlock.SKY, l, j1, i1) <= 0) {
            	final EntityHuman entityhuman = this.findNearbyPlayer((double) l + 0.5D, (double) j1 + 0.5D, (double) i1 + 0.5D, 8.0D);
    ...
    public boolean d(int i, int j, int k, boolean flag) {
    	...
    	final float f = biomebase.a(i, j, k);
        ...
        if (enumskyblock == EnumSkyBlock.SKY && this.i(i, j, k)) {
            ...
            if (i1 >= 15) {
                ...
                for (int j1 = 0; j1 < 6; ++j1) {
                	final int k1 = i + Facing.b[j1];
                	final int l1 = j + Facing.c[j1];
                	final int i2 = k + Facing.d[j1];
                	final int j2 = this.b(enumskyblock, k1, l1, i2) - i1;
    ...
    public boolean c(EnumSkyBlock enumskyblock, int i, int j, int k, Chunk chunk, List<Chunk> neighbors) { // PaperSpigot
        ...
        if (chunk == null /*|| !chunk.areNeighborsLoaded(1)*/ /* !this.areChunksLoaded(i, j, k, 17)*/) {
            ...
            final int j1 = this.b(enumskyblock, i, j, k);
            final int k1 = this.a(i, j, k, enumskyblock);
            int l1;
            int i2;
            int j2;
            int k2;
                        ...
                        if (l2 > 0) {
                            ...
                            if (j3 + l3 + k3 < 17) {
                                for (int i4 = 0; i4 < 6; ++i4) {
                                	final int j4 = i2 + Facing.b[i4];
                                	final int k4 = j2 + Facing.c[i4];
                                	final int l4 = k2 + Facing.d[i4];
                                	final int i5 = Math.max(1, this.getType(j4, k4, l4).k());
            ...
            while (l < i1) {
                ...
                if (i3 != l2) {
                    ...
                    if (i3 > l2) {
                        ...
                        l3 = Math.abs(j2 - j);
                        k3 = Math.abs(k2 - k);
                        final boolean flag = i1 < this.I.length - 6;
    ...
    public boolean updateLight(final EnumSkyBlock enumskyblock, final int x, final int y, final int z) {
    	//PandaSpigot start - optimize light update for already known chunks this is for legacy
    	final Chunk chunk = this.getChunkIfLoaded(x >> 4, z >> 4);
    	// Paper start - optimize light updates where chunk is known
    	return updateBrightness(enumskyblock, x, y, z, chunk);
    }
    public boolean updateBrightness(EnumSkyBlock enumskyblock, final int x, final int y, final int z, Chunk chunk) {
        //final Chunk chunk = this.getChunkIfLoaded(x >> 4, z >> 4);
        ...
        for (int cx = (x >> 4) - 1; cx <= (x >> 4) + 1; ++cx) {
            for (int cz = (z >> 4) - 1; cz <= (z >> 4) + 1; ++cz) {
                if (cx != x >> 4 && cz != z >> 4) {
                	final Chunk neighbor = this.getChunkIfLoaded(cx, cz);
                    if (neighbor != null) {
    ...
    public List<Entity> getEntities(Entity entity, AxisAlignedBB axisalignedbb, IEntitySelector ientityselector) {
    	final List<Entity> arraylist = new ArrayList<>();
    ...
    public List<Entity> a(Class oclass, AxisAlignedBB axisalignedbb) {
        return this.a(oclass, axisalignedbb, (IEntitySelector) null);
    }

    public List<Entity> a(Class oclass, AxisAlignedBB axisalignedbb, IEntitySelector ientityselector) {
    	final int i = MathHelper.floor((axisalignedbb.a - 2.0D) / 16.0D);
    	final int j = MathHelper.floor((axisalignedbb.d + 2.0D) / 16.0D);
    	final int k = MathHelper.floor((axisalignedbb.c - 2.0D) / 16.0D);
    	final int l = MathHelper.floor((axisalignedbb.f + 2.0D) / 16.0D);
    	final List<Entity> arraylist = new ArrayList<Entity>();
    ...
    public Entity a(Class oclass, AxisAlignedBB axisalignedbb, Entity entity) {
    	final List<Entity> list = this.a(oclass, axisalignedbb);
        Entity entity1 = null;
        double d0 = Double.MAX_VALUE;

        for (Entity entity2 : list) {
            if (entity2 != entity) {
            	final double d1 = entity.f(entity2);
    ...
    public boolean mayPlace(Block block, int i, int j, int k, boolean flag, int l, Entity entity, ItemStack itemstack) {
    	final Block block1 = this.getType(i, j, k);
    	final AxisAlignedBB axisalignedbb = flag ? null : block.a(this, i, j, k);

        // CraftBukkit start - store default return
    	final boolean defaultReturn = axisalignedbb != null && !this.a(axisalignedbb, entity) ? false : (block1.getMaterial() == Material.ORIENTABLE && block == Blocks.ANVIL ? true : block1.getMaterial().isReplaceable() && block.canPlace(this, i, j, k, l, itemstack));

        // CraftBukkit start
    	final BlockCanBuildEvent event = new BlockCanBuildEvent(this.getWorld().getBlockAt(i, j, k), CraftMagicNumbers.getId(block), defaultReturn);
        this.getServer().getPluginManager().callEvent(event);
    ...
    public PathEntity findPath(Entity entity, Entity entity1, float f, boolean flag, boolean flag1, boolean flag2, boolean flag3) {
        final int i = MathHelper.floor(entity.locX);
        final int j = MathHelper.floor(entity.locY + 1.0D);
    ...
    public PathEntity a(Entity entity, int i, int j, int k, float f, boolean flag, boolean flag1, boolean flag2, boolean flag3) {
        ...
        final int i1 = MathHelper.floor(entity.locY);
        final int j1 = MathHelper.floor(entity.locZ);
        final int k1 = (int) (f + 8.0F);
        final int l1 = l - k1;
        final int i2 = i1 - k1;
        final int j2 = j1 - k1;
        final int k2 = l + k1;
    ...
    public int getHighestNeighborSignal(int i, int j, int k) {
        ...
        for (int i1 = 0; i1 < 6; ++i1) {
        	final int j1 = this.getBlockFacePower(i + Facing.b[i1], j + Facing.c[i1], k + Facing.d[i1], i1);
    ...
    public EntityHuman findNearbyPlayer(double d0, double d1, double d2, double d3) {
        ...
        for (double x = d0 - d3; x <= d0 + d3; x += 16.0D) {
        	for (double z = d2 - d3; z <= d2 + d3; z += 16.0D) {
        		final Chunk chunk = getChunkIfLoaded((int)x >> 4, (int)z >> 4);
        		...
        		for (EntityPlayer entityPlayer : chunk.playersInChunk) {
		            ...
		            // CraftBukkit end
		            final double d5 = entityPlayer.e(d0, d1, d2);
    ...
    public EntityHuman findNearbyPlayerWhoAffectsSpawning(double x, double y, double z, double radius) {
        ...
        EntityHuman entityHuman = null;
        for (EntityHuman nearestPlayer : this.players) {
            if (nearestPlayer == null || !nearestPlayer.isAlive() || !nearestPlayer.affectsSpawning) {
                continue;
            }

            final double distance = nearestPlayer.e(x, y, z);
    ...
    public void setSpawn(int x, int y, int z, float yaw, float pitch) {
        this.worldData.setSpawn(x, y, z, yaw, pitch);
    }
    ...
    public void updateAdjacentComparators(int i, int j, int k, Block block) {
        for (int l = 0; l < 4; ++l) {
            ...
            final Block block1 = this.getType(i1, j, j1);
            ...
            if (Blocks.REDSTONE_COMPARATOR_OFF.e(block1)) {
                ...
                i1 += Direction.a[l];
                j1 += Direction.b[l];
                final Block block2 = this.getType(i1, j, j1);
    ...
    public float B(int i, int j, int k) {
        float f = 0.0F;
        final boolean flag = this.difficulty == EnumDifficulty.HARD;

        if (this.isLoaded(i, j, k)) {
        	final float f1 = this.y();
    ...
    public void X() {
        ...
        while (iterator.hasNext()) {
        	final IWorldAccess iworldaccess = (IWorldAccess) iterator.next();
