...
public class EntityFireworks extends Entity implements IEntitySpecificSpawnPacket, ITrack {
    ...
    private void updateYawPitch() {
    	final float f1 = (float)Math.sqrt(this.motX * this.motX + this.motZ * this.motZ);
        this.yaw = (float) Math.toDegrees(Math.atan2(this.motX, this.motZ));
        this.pitch = (float) Math.toDegrees(Math.atan2(this.motY, (double) f1));

        this.pitch = normalizeAngle(this.pitch, this.lastPitch);
        this.yaw = normalizeAngle(this.yaw, this.lastYaw);
    }
    private float normalizeAngle(float angle, float reference) {
