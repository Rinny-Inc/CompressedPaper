...
public abstract class Entity {
    ...
    // CraftBukkit start
    //private static final byte CURRENT_LEVEL = 2; // Rinny moved into EntityInsentient since its where it's used
    public static LightRandom SHARED_RANDOM = new LightRandom();
    public boolean isLevelAtLeast(NBTTagCompound tag, int level) { // Rinny - static boolean > public boolean // TODO check if it does shit
    ...
    public static int entityCount = 1; // value not init usually
    private int id;
    public double j;
    public boolean k;
    ...
    protected boolean inPortal; // deobfuscate inPortal
    protected int ao;
    public int dimension;
    protected int aq;
    private boolean invulnerable;
    public UUID uniqueID; // CraftBukkit - protected -> public
    //public EnumEntitySize as; // Rinny - Removed this because its not inculded in higher version
    ...
    private Chunk[] prevInBlockChunks;
    private int[] previousInBlockJ;
    private int[] previousInBlockK;
    private int[] previousInBlockL;
    private double previousInBlockX;
    private double previousInBlockY;
	private double previousInBlockZ;

    public Entity(World world) {
        this.id = entityCount++;
        this.j = 1.0D;
        this.boundingBox = AxisAlignedBB.a(0.0D, 0.0D, 0.0D, 0.0D, 0.0D, 0.0D);
        this.prevInBlockChunks = new Chunk[8];
        this.previousInBlockJ = new int[8];
        this.previousInBlockK = new int[8];
        this.previousInBlockL = new int[8];
        this.previousInBlockX = -1000000.0D;
        this.previousInBlockY = -1000000.0D;
        this.previousInBlockZ = -1000000.0D;
        this.J = true;
        this.width = 0.6F;
        this.length = 1.8F;
        this.d = 1;
        ...
        //this.as = EnumEntitySize.SIZE_2; // Rinny - Removed this because its not inculded in higher version
        this.world = world;
        ...
        this.c();
        //this.datawatcher.registrationLocked = true; // PandaSpigot // Badlion
    }
    ...
    public void C() {
            ...
            if (this.inPortal) {
                if (true || minecraftserver.getAllowNether()) { // CraftBukkit
                    if (this.vehicle == null && this.ao++ >= i) {
                        ...
                        final byte b0 = (byte) (this.world.worldProvider.dimension == -1 ? 0 : -1);
                        this.b(b0);
                    }
                    this.inPortal = false;
                }
        ...
        if (this.isSprinting() && !this.M()) {
            ...
            final Block block = this.world.getType(j, i, k);
    ...
    protected void E() {
    	if (!this.fireProof) {
            ...
            // Rinny - moved this into EntityLiving
    ...
    public boolean c(double d0, double d1, double d2) {
        ...
        final List<AxisAlignedBB> list = this.world.getCubes(this, axisalignedbb);
     ...
     */
    // sathonay - start
    // TODO: ensure that there's no missed chunk loading
    private int lastChunkX = Integer.MIN_VALUE;
    private int lastChunkZ = Integer.MIN_VALUE;
    public void loadChunks() {
        int cx = (int) locX >> 4;
        int cz = (int) locZ >> 4;
        if (lastChunkX != cx || lastChunkZ != cz) {
            for (; cx <= (int) (locX + motX) >> 4; ++cx) {
                for (; cz <= (int) (locZ + motZ) >> 4; ++cz) {
    ...
    public void move(double d0, double d1, double d2) {
        ...
        if (this.X) {
            ...
            if (this.W > 0.0F && flag1 && (flag || this.V < 0.05F) && (d6 != d0 || d8 != d2)) {
                ...
                d1 = (double) this.W;
                d2 = d8;
                final AxisAlignedBB axisalignedbb1 = this.boundingBox.clone();
            ...
            // CraftBukkit start
            if ((this.positionChanged) && (this.getBukkitEntity() instanceof Vehicle vehicle)) { // Rinny
            ...
            if (this.g_() && !flag && this.vehicle == null) {
                ...
                final int i1 = MathHelper.floor(this.locZ);
                Block block = this.world.getType(l, k, i1);
                final int j1 = this.world.getType(l, k - 1, i1).b();
                ...
                this.P = (float) ((double) this.P + Math.sqrt(d10 * d10 + d12 * d12) * 0.6D);
                this.Q = (float) ((double) this.Q + Math.sqrt(d10 * d10 + d11 * d11 + d12 * d12) * 0.6D);
                if (this.Q > (float) this.d && block.getMaterial() != Material.AIR) {
                    this.d = (int) this.Q + 1;
                    if (this.M()) {
                    	final float f = Math.min((float)Math.sqrt(this.motX * this.motX * 0.20000000298023224D + this.motY * this.motY + this.motZ * this.motZ * 0.20000000298023224D) * 0.35F, 1.0F);
    ...
    protected void I() {
    	final int i = MathHelper.floor(this.boundingBox.a + 0.001D);
    	final int j = MathHelper.floor(this.boundingBox.b + 0.001D);
    	final int k = MathHelper.floor(this.boundingBox.c + 0.001D);
    	final int l = MathHelper.floor(this.boundingBox.d - 0.001D);
    	final int i1 = MathHelper.floor(this.boundingBox.e - 0.001D);
    	final int j1 = MathHelper.floor(this.boundingBox.f - 0.001D);
    ...
    public boolean N() {
    	...
    	if (this.fallDistance > 3.0F && this.world.a(bb, Material.WEB, this)) {
    		this.fallDistance = 0.0F;
    		return false;
    	}
    	// Rinny end
        if (this.world.a(bb, Material.WATER, this)) {
            if (!this.inWater && !this.justCreated) {
            	final float f = Math.min((float)Math.sqrt(this.motX * this.motX * 0.20000000298023224D + this.motY * this.motY + this.motZ * this.motZ * 0.20000000298023224D) * 0.2F, 1.0F);

                this.makeSound(this.O(), f, 1.0F + (this.random.nextFloat() - this.random.nextFloat()) * 0.4F);
                final float f1 = (float) MathHelper.floor(this.boundingBox.b);

                // Rinny - dont init float 24/7
                float f2;
                float f3;
                // Rinny end
    ...
    public boolean a(Material material) {
    	final double d0 = this.locY + (double) this.getHeadHeight();
        ...
        if (block.getMaterial() == material) {
        	final float f = BlockFluids.b(this.world.getData(i, j, k)) - 0.11111111F;
        	final float f1 = (float) (j + 1) - f;
    ...
    public void a(float f, float f1, float f2) {
        ...
        if (f3 >= 1.0E-4F) {
            f3 = Math.max((float)Math.sqrt(f3), 1.0F);
            f3 = f2 / f3;
            f *= f3;
            f1 *= f3;
            final float radians = this.yaw * 3.1415927F / 180.0F; // Rinny - calculate only once
            final float f4 = MathHelper.sin(radians);
            final float f5 = MathHelper.cos(radians);
    ...
    public float d(float f) {
        ...
        if (this.world.isLoaded(i, 0, j)) {
        	final double d0 = (this.boundingBox.e - this.boundingBox.b) * 0.66D;
        	final int k = MathHelper.floor(this.locY - (double) this.height + d0);
    ...
    public void setLocation(double d0, double d1, double d2, float f, float f1) {
        ...
        this.V = 0.0F;
        final double d3 = (double) (this.lastYaw - f);
    ...
    public float e(Entity entity) {
    	...
    	final float f1 = (float) (this.locY - entity.locY);
    	final float f2 = (float) (this.locZ - entity.locZ);
    ...
    public double f(Entity entity) {
        ...
        final double d1 = this.locY - entity.locY;
        final double d2 = this.locZ - entity.locZ;
    ...
    public void collide(Entity entity) {
        if (entity.passenger != this && entity.vehicle != this) {
            ...
            if (d2 >= 0.009999999776482582D) {
                ...
                d0 /= d2;
                d1 /= d2;
                final double d3 = Math.min(1.0D / d2, 1.0D);
    ...
    public void f(NBTTagCompound nbttagcompound) {
        try {
        	...
        	final NBTTagList nbttaglist1 = nbttagcompound.getList("Motion", 6);
        	final NBTTagList nbttaglist2 = nbttagcompound.getList("Rotation", 5);
            ...
            // CraftBukkit start
            if (this instanceof EntityLiving entity) { // Rinny
                this.ticksLived = nbttagcompound.getInt("Spigot.ticksLived");

                // Reset the persistence for tamed animals
                if (entity instanceof EntityTameableAnimal entityinsentient && !isLevelAtLeast(nbttagcompound, 2) && !nbttagcompound.getBoolean("PersistenceRequired")) { // Rinny
    ...
    protected NBTTagList a(double... adouble) {
    	...
    	final double[] adouble1 = adouble;
    	final int i = adouble.length;
    ...
    protected NBTTagList a(float... afloat) {
    	final NBTTagList nbttaglist = new NBTTagList();
    	final float[] afloat1 = afloat;
    	final int i = afloat.length;
    ...
    /*public boolean inBlock() {
    	final float w = this.width * 0.8F; // Rinny - calculate width only once
        for (int i = 0; i < 8; ++i) {
            float f = ((float) ((i >> 0) % 2) - 0.5F) * w;
            float f1 = ((float) ((i >> 1) % 2) - 0.5F) * 0.1F;
            float f2 = ((float) ((i >> 2) % 2) - 0.5F) * w;
	...
	public boolean inBlock() {
		if (this.previousInBlockX == this.locX && this.previousInBlockZ == this.locZ && this.previousInBlockY == this.locY) {
			int j = 0;
			for (; j < 8 && this.prevInBlockChunks[j] != null; j++) {
				if (this.prevInBlockChunks[j].getType(this.previousInBlockJ[j] & 0xF, this.previousInBlockK[j], this.previousInBlockL[j] & 0xF).r()) {
					return true; 
				} 
			}
			return inBlockHelper(j);
		} 
		for (int i = 0; i < 8; i++) {
			this.prevInBlockChunks[i] = null; 
		}
		return inBlockHelper(0);
	}
      
	private boolean inBlockHelper(int start) {
		Chunk chunk = null;
		int prevChunkX = 0;
		int prevChunkZ = 0;
		this.previousInBlockX = this.locX;
		this.previousInBlockY = this.locY;
		this.previousInBlockZ = this.locZ;
		
		final float w = this.width * 0.8F;
		for (int i = start; i < 8; i++) {
			float f = ((i % 2) - 0.5F) * w;
			float f1 = (((i >> 1) % 2) - 0.5F) * 0.1F;
			float f2 = (((i >> 2) % 2) - 0.5F) * w;
			int j = MathHelper.floor(this.locX + f);
			int k = MathHelper.floor(this.locY + getHeadHeight() + f1);
			int l = MathHelper.floor(this.locZ + f2);
			
			this.previousInBlockJ[i] = j;
			this.previousInBlockK[i] = k;
			this.previousInBlockL[i] = l;
			
			if (chunk == null || (prevChunkX != j >> 4 || prevChunkZ != l >> 4)) {
				prevChunkX = j >> 4;
        		prevChunkZ = l >> 4;
                chunk = this.world.getChunkAt(prevChunkX, prevChunkZ);
			}
	        
			this.prevInBlockChunks[i] = chunk;
			if (chunk.getType(j & 0xF, k, l & 0xF).r()) {
				return true; 
			}
		} 
		return false;
    ...
    public void ab() {
        if (this.vehicle.dead) {
            ...
            if (this.vehicle != null) {
                ...
                d0 = Math.max(-f, Math.min(d0, f));
                d1 = Math.max(-f, Math.min(d1, f));
    ...
    public void setPassengerOf(Entity entity) {
    	...
    	final Entity originalVehicle = this.vehicle;
    	final Entity originalPassenger = this.vehicle == null ? null : this.vehicle.passenger;
        ...
        if (entity == null) {
            if (this.vehicle != null) {
                ...
                // CraftBukkit end
                EntityDismountEvent event = new EntityDismountEvent(this.getBukkitEntity(), this.vehicle.getBukkitEntity());
    ...
    public void ah() {
        if (this.portalCooldown > 0) {
        	...
        	final double d1 = this.lastZ - this.locZ;
    ...
    protected boolean j(double d0, double d1, double d2) {
    	...
    	final double d3 = d0 - (double) i;
    	final double d4 = d1 - (double) j;
    	final double d5 = d2 - (double) k;
    	final List list = this.world.a(this.boundingBox);

        if (list.isEmpty() && !this.world.q(i, j, k)) {
            return false;
        } else {
        	final boolean flag = !this.world.q(i - 1, j, k);
        	final boolean flag1 = !this.world.q(i + 1, j, k);
            //boolean flag2 = !this.world.q(i, j - 1, k);
        	final boolean flag3 = !this.world.q(i, j + 1, k);
        	final boolean flag4 = !this.world.q(i, j, k - 1);
        	final boolean flag5 = !this.world.q(i, j, k + 1);
            byte b0 = 3;
            double d6 = 9999.0D;
            ...
            // Rinny start
            switch (b0) {
            	case 0 -> this.motX = (double) (-f);
            	case 1 -> this.motX = (double) f;
            	case 2 -> this.motY = (double) (-f);
            	case 3 -> this.motY = (double) f;
            	case 4 -> this.motZ = (double) (-f);
            	case 5 -> this.motZ = (double) f;
            }
            // Rinny end
            return true;
        }
    ...
    public void teleportTo(Location exit, boolean portal) {
        if (true) {
            ...
            final boolean before = worldserver1.chunkProviderServer.forceChunkLoad;
            ...
            // CraftBukkit end
            final Entity entity = EntityTypes.createEntityByName(EntityTypes.b(this), worldserver1);
