...
public class CraftingManager {
    ...
    public ShapedRecipes registerShapedRecipe(ItemStack itemstack, Object... aobject) {
        ...
        for (hashmap = new HashMap<>(); i < aobject.length; i += 2) {
            ...
            // Rinny start - put var
            if (aobject[i + 1] instanceof Item item) {
                itemstack1 = new ItemStack(item);
            } else if (aobject[i + 1] instanceof Block block) {
                itemstack1 = new ItemStack(block, 1, 32767);
            } else if (aobject[i + 1] instanceof ItemStack stack) {
                itemstack1 = stack;
            }
            // Rinny end
        ...
        for (int i1 = 0; i1 < j * k; ++i1) {
        	...
        	// Rinny
        	char c0 = s.charAt(i1);
            ItemStack stack = hashmap.get(c0);
            aitemstack[i1] = (stack != null) ? stack.cloneItemStack() : null;
            // Rinny
        }
    ...
    public List getRecipes() {
        return Collections.unmodifiableList(this.recipes); // Rinny - add Collections.unmodifiableList
    }
}
