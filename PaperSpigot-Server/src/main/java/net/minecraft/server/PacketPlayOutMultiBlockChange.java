...
public class PacketPlayOutMultiBlockChange extends Packet {
    ...
    public PacketPlayOutMultiBlockChange(int chunkX, int chunkZ, int[] xCoords, int[] yCoords, int[] zCoords, int[] blockIds, int[] dataIds) {
    	if (xCoords.length != yCoords.length || zCoords.length != yCoords.length) {
    		throw new RuntimeException("Invalid data"); 
    	}
    	short[] shorts = new short[xCoords.length];
    	for (int k = 0; k < xCoords.length; k++) {
    		shorts[k] = (short)(xCoords[k] << 12 | zCoords[k] << 8 | yCoords[k]); 
    	}
    	this.ashort = shorts;
    	this.b = new ChunkCoordIntPair(chunkX, chunkZ);
    	this.d = xCoords.length;
    	int j = 4 * xCoords.length;
    	try {
    		ByteArrayOutputStream bytearrayoutputstream = new ByteArrayOutputStream(j);
    		DataOutputStream dataoutputstream = new DataOutputStream(bytearrayoutputstream);
    		this.blocks = new int[xCoords.length];
    		for (int i = 0; i < xCoords.length; i++) {
    			dataoutputstream.writeShort(this.ashort[i]);
