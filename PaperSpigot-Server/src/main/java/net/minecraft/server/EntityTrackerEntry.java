...
public class EntityTrackerEntry {
    ...
    // Rinny
    @NotNull
    public final Map<EntityPlayer, Boolean> trackedPlayerMap =
        new IdentityHashMap<EntityPlayer, Boolean>();
    ...
    public boolean equals(Object object) {
        return object instanceof EntityTrackerEntry
            ? ((EntityTrackerEntry) object).tracker.getId() ==
                  this.tracker.getId()
            : false;
    }
    ...
    public void track() {
        ...
        if (!this.isMoving || this.tracker.e(this.q, this.r, this.s) > 16.0D) {
            ...
            this.isMoving = true;
            this.n = true;
            this.tracker.world.performOnInRangePlayers(
                this.tracker,
                this.b,
                this::updatePlayer
            );
        ...
        if (
            ...
            (this.tracker.vehicle != null && this.m % 60 == 0)
        ) {
            PacketPlayOutMount packetPlayOutMount;
            Packet attach = new PacketPlayOutAttachEntity(
                0,
                this.tracker,
            ...
            if (this.tracker.vehicle == null) {
                ...
                packetPlayOutMount = new PacketPlayOutMount(
                    this.tracker.vehicle,
                    this.tracker
                );
            }
            for (EntityPlayer entityplayer : this.trackedPlayers) {
                if (
                    entityplayer.playerConnection.networkManager.getVersion() <
                    107
                ) {
                ...
                entityplayer.playerConnection.sendPacket(
                    (Packet) packetPlayOutMount
                );
            }
            this.w = this.tracker.vehicle;
        }

        if (this.tracker instanceof EntityItemFrame i3) {
            // CraftBukkit - Moved below, should always enter this block // Rinny add var
            final ItemStack i4 = i3.getItem();

            if (
                this.m % 10 == 0 &&
                i4 != null &&
                i4.getItem() instanceof ItemWorldMap
            ) {
                // CraftBukkit - Moved this.m % 10 logic here so item frames do not enter the other blocks
                final WorldMap i6 = Items.MAP.getSavedMap(
                    i4,
                    this.tracker.world
                );
                Packet packet; // Rinny
            ...
            if (this.tracker.vehicle == null) {
                ...
                final int l = MathHelper.d(
                    (this.tracker.yaw * 256.0F) / 360.0F
                );
                final int i1 = MathHelper.d(
                    (this.tracker.pitch * 256.0F) / 360.0F
                );
                ...
                if (this.m > 0) {
                    // PaperSpigot - Move up // OLD => || this.tracker instanceof EntityArrow
                    final boolean flag =
                        Math.abs(j1) >= 4 ||
                        Math.abs(k1) >= 4 ||
                        Math.abs(l1) >= 4 ||
                    ...
                    final boolean flag1 =
                        Math.abs(l - this.yRot) >= 4 ||
                        Math.abs(i1 - this.xRot) >= 4;
                    if (flag1) {
                        this.yRot = l;
                        this.xRot = i1;
                    }
                    ...
                    if (
                        ...
                        if (flag && flag1) {
                            packet = new PacketPlayOutRelEntityMoveLook(
                                ...
                                (byte) i1,
                                tracker.onGround
                            ); // Spigot - protocol patch
                        } else if (flag) {
                            packet = new PacketPlayOutRelEntityMove(
                                this.tracker.getId(),
                                (byte) j1,
                                (byte) k1,
                                (byte) l1,
                            ...
                            packet = new PacketPlayOutEntityLook(
                                this.tracker.getId(),
                                (byte) l,
                                (byte) i1,
                                tracker.onGround
                ...
                if (this.u) {
                    ...
                    if (
                        ...
                        this.broadcast(
                            new PacketPlayOutEntityVelocity(
                                this.tracker.getId(),
                                this.j,
                                this.k,
                                this.l
                ...
                if (packet != null) {
                    if (packet instanceof PacketPlayOutEntityTeleport) {
                        ...
                        for (Map.Entry<
                            EntityPlayer,
                            Boolean
                        > viewer : trackedPlayerMap.entrySet()) {
                            if (viewer.getValue()) {
                ...
                final boolean flag2 =
                    Math.abs(i - this.yRot) >= 4 ||
                    Math.abs(j - this.xRot) >= 4;
            ...
            i = MathHelper.d(
                (this.tracker.getHeadRotation() * 256.0F) / 360.0F
            );
            if (Math.abs(i - this.i) >= 4) {
                this.broadcast(
                    new PacketPlayOutEntityHeadRotation(this.tracker, (byte) i)
                );
                this.i = i;
            }
        ...
        if (this.tracker.velocityChanged) {
            ...
            if (!cancelled) {
                this.broadcastIncludingSelf(
                    new PacketPlayOutEntityVelocity(this.tracker)
                );
            }
            // CraftBukkit end
    ...
    private void b() {
        ...
        if (datawatcher.a()) {
            if (this.tracker instanceof EntityPlayer player) {
                PacketPlayOutEntityMetadata broadcast =
                    new PacketPlayOutEntityMetadata(player, false);
        ...
        if (this.tracker instanceof EntityLiving living) {
            final AttributeMapServer attributemapserver =
                (AttributeMapServer) living.getAttributeMap();
            ...
            if (!set.isEmpty()) {
                ...
                // CraftBukkit end
                this.broadcastIncludingSelf(
                    new PacketPlayOutUpdateAttributes(this.tracker.getId(), set)
                );
            }
    ...
    public void a() {
        for (EntityPlayer entityplayer : this.trackedPlayers) {
            this.owner.unlinkPlayer(entityplayer, this);
    ...
    public void updatePlayer(EntityPlayer entityplayer) {
        if (entityplayer != this.tracker) {
            ...
            if (
                ...
                if (
                    !this.trackedPlayers.contains(entityplayer) &&
                    (this.d(entityplayer) || this.tracker.attachedToPlayer)
                ) {
                    // CraftBukkit start - respect vanish API
                    if (
                        !entityplayer
                            .getBukkitEntity()
                            .canSee(this.tracker.getBukkitEntity())
                    ) {
                        return;
                    }

                    entityplayer.removeQueue.remove(
                        Integer.valueOf(this.tracker.getId())
                    );
                    // CraftBukkit end

                    this.trackedPlayerMap.put(entityplayer, true);
                    this.owner.linkPlayer(entityplayer, this); // Rinny - keep reverse index in sync
                    ...
                    if (tracker instanceof EntityPlayer) {
                        ...
                        if (
                            ...
                            entityplayer.playerConnection.sendPacket(
                                new PacketPlayOutPlayerInfo(
                                    (EntityPlayer) tracker,
                                    PlayerInfo.UPDATE_DISPLAY_NAME
                                )
                            );
                        }
                    }
                    ...
                    if (!this.tracker.getDataWatcher().d()) {
                        PacketPlayOutEntityMetadata meta =
                            new PacketPlayOutEntityMetadata(this.tracker, true);
                        if (this.tracker instanceof EntityPlayer) {
                    ...
                    if (this.tracker instanceof EntityLiving) {
                        AttributeMapServer attributemapserver =
                            (AttributeMapServer) (
                                (EntityLiving) this.tracker
                            ).getAttributeMap();
                        Collection collection = attributemapserver.c();

                        // CraftBukkit start - If sending own attributes send scaled health instead of current maximum health
                        if (this.tracker.getId() == entityplayer.getId()) {
                            ((EntityPlayer) this.tracker)
                                .getBukkitEntity()
                        ...
                        if (!collection.isEmpty()) {
                            entityplayer.playerConnection.sendPacket(
                                new PacketPlayOutUpdateAttributes(
                                    this.tracker.getId(),
                                    collection
                                )
                            );
                    ...
                    if (
                        this.tracker instanceof EntityInsentient insentient &&
                        insentient.getLeashHolder() != null
                    ) {
                        entityplayer.playerConnection.sendPacket(
                            new PacketPlayOutAttachEntity(
                                1,
                                this.tracker,
                                insentient.getLeashHolder()
                            )
                        );
                    }
                        ...
                        if (entityhuman.isSleeping()) {
                            entityplayer.playerConnection.sendPacket(
                                new PacketPlayOutBed(
                                    entityhuman,
                                    MathHelper.floor(this.tracker.locX),
                                    MathHelper.floor(this.tracker.locY),
                                    MathHelper.floor(this.tracker.locZ)
                                )
                            );
                        }
                    }

                    if (this.tracker instanceof EntityLiving entityliving) {
                        // Rinny
                        ...
                        this.i = MathHelper.d(
                            (entityliving.getHeadRotation() * 256.0F) / 360.0F
                        );
                        entityplayer.playerConnection.sendPacket(
                        ...
                        Iterator iterator = entityliving
                            .getEffects()
                            .iterator();
                ...
                this.owner.unlinkPlayer(entityplayer, this); // Rinny - keep reverse index in sync
                entityplayer.d(this.tracker);
            }
        }
    }
    ...
    private boolean d(EntityPlayer entityplayer) {
        return entityplayer
            .r()
            .getPlayerChunkMap()
            .a(entityplayer, this.tracker.ah, this.tracker.aj);
    }
    ...
    private Packet c() {
        ...
        // nPaper
        if (this.tracker instanceof IEntitySpecificSpawnPacket specificPacket) {
            // Rinny add var
        ...
        if (
            this.tracker instanceof IAnimal ||
