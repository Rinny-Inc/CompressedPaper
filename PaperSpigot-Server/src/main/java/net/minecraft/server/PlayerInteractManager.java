...
public class PlayerInteractManager {
    ...
    public void a() {
        ...
        if (this.j) {
            final int j = this.currentTick - this.n;
            final Block block = this.world.getType(this.k, this.l, this.m);
        ...
        if (this.d) {
        	final Block block1 = this.world.getType(this.f, this.g, this.h);
            ...
            if (block1.getMaterial() == Material.AIR) {
            	...
            	final int k = this.currentTick - this.lastDigTick;
    ...
    public void dig(int i, int j, int k, int l) {
        ...
        if (!this.gamemode.isAdventure() || this.player.d(i, j, k)) {
        	final PlayerConnection cachedPlayerConnection = ((EntityPlayer) this.player).playerConnection;
            if (event.isCancelled()) {
                // Let the client know the block still exists
            	cachedPlayerConnection.sendPacket(new PacketPlayOutBlockChange(i, j, k, this.world));
                // Update any tile entity data for this block
                final TileEntity tileentity = this.world.getTileEntity(i, j, k);
                if (tileentity != null) {
                	cachedPlayerConnection.sendPacket(tileentity.getUpdatePacket());
                }
                return;
            }
            // CraftBukkit end
            if (this.isCreative()) {
                ...
                float f = 1.0F;
                //final Block block = this.world.getType(i, j, k); // Rinny - moved up
                // CraftBukkit start - Swings at air do *NOT* exist.
                final boolean isDenied = event.useInteractedBlock() == Event.Result.DENY; // Rinny - cache it
                if (isDenied) {
                    // If we denied a door from opening, we need to send a correcting update to the client, as it already opened the door.
                    if (block == Blocks.WOODEN_DOOR) {
                        // For some reason *BOTH* the bottom/top part have to be marked updated.
                        final boolean bottom = (this.world.getData(i, j, k) & 8) == 0;
                        cachedPlayerConnection.sendPacket(new PacketPlayOutBlockChange(i, j, k, this.world));
                        cachedPlayerConnection.sendPacket(new PacketPlayOutBlockChange(i, j + (bottom ? 1 : -1), k, this.world));
                ...
                final org.bukkit.event.block.BlockDamageEvent blockEvent = CraftEventFactory.callBlockDamageEvent(this.player, i, j, k, this.player.inventory.getItemInHand(), f >= 1.0f);
    ...
    public void a(int i, int j, int k) {
        if (i == this.f && j == this.g && k == this.h) {
            ...
            if (block.getMaterial() != Material.AIR) {
            	final float f = block.getDamage(this.player, this.player.world, i, j, k) * (float) (l + 1);
    ...
    public boolean breakBlock(int i, int j, int k) {
        ...
        if (this.player instanceof EntityPlayer) {
            ...
            if (world.getTileEntity(i, j, k) == null) {
            	final PacketPlayOutBlockChange packet = new PacketPlayOutBlockChange(i, j, k, this.world);
            ...
            //                             Adventure mode pre-cancel                                     Sword + Creative mode pre-cancel
            event.setCancelled(this.gamemode.isAdventure() && !this.player.d(i, j, k) || (this.gamemode.d() && this.player.be() != null && this.player.be().getItem() instanceof ItemSword));
            ...
            final Block nmsBlock = this.world.getType(i, j, k);
		...
		if (this.isCreative()) {
			...
			final ItemStack itemstack = this.player.bF();
			final boolean flag1 = this.player.a(block);
    ...
    public boolean useItem(EntityHuman entityhuman, World world, ItemStack itemstack) {
        ...
        final int itemIndex = entityhuman.inventory.itemInHandIndex; // Rinny - cache
        entityhuman.inventory.items[itemIndex] = itemstack1;
        if (this.isCreative()) {
            itemstack1.count = i;
            if (itemstack1.g()) {
        ...
        if (itemstack1.count == 0) {
            entityhuman.inventory.items[itemIndex] = null;
        }
        if (!entityhuman.by()) {
    ...
    public boolean interact(EntityHuman entityhuman, World world, ItemStack itemstack, int i, int j, int k, int l, float f, float f1, float f2) {
        ...
        // Interract event */
        final Block block = world.getType(i, j, k);
        ...
        if (block != Blocks.AIR) {
            ...
            if (event.useInteractedBlock() == Event.Result.DENY) {
                // If we denied a door from opening, we need to send a correcting update to the client, as it already opened the door.
                if (block == Blocks.WOODEN_DOOR) {
                	final boolean bottom = (world.getData(i, j, k) & 8) == 0;
