...
public class ContainerEnchantTable extends Container {
        ...
        if (iinventory == this.enchantSlots) {
            ...
            if (itemstack != null) { // CraftBukkit - relax condition
                ...
                if (!this.world.isStatic) {
                    ...
                    final Block block = Blocks.BOOKSHELF;
                    for (j = -1; j <= 1; ++j) {
                        for (int k = -1; k <= 1; ++k) {
                            if ((j != 0 || k != 0) && this.world.isEmpty(this.x + k, this.y, this.z + j) && this.world.isEmpty(this.x + k, this.y + 1, this.z + j)) {
                                if (this.world.getType(this.x + k * 2, this.y, this.z + j * 2) == block) {
                                    ++i;
                                }

                                if (this.world.getType(this.x + k * 2, this.y + 1, this.z + j * 2) == block) {
                                    ++i;
                                }

                                if (k != 0 && j != 0) {
                                    if (this.world.getType(this.x + k * 2, this.y, this.z + j) == block) {
                                        ++i;
                                    }

                                    if (this.world.getType(this.x + k * 2, this.y + 1, this.z + j) == block) {
                                        ++i;
                                    }

                                    if (this.world.getType(this.x + k, this.y, this.z + j * 2) == block) {
                                        ++i;
                                    }

                                    if (this.world.getType(this.x + k, this.y + 1, this.z + j * 2) == block) {
                                        ++i;
                                    }
                                }
                            }
    ...
    public boolean a(EntityHuman entityhuman, int i) {
        ...
        if (this.costs[i] > 0 && itemstack != null && (entityhuman.expLevel >= this.costs[i] || entityhuman.abilities.canInstantlyBuild)) {
            if (!this.world.isStatic) {
                List<EnchantmentInstance> list = EnchantmentManager.b(this.l, itemstack, this.costs[i]);
