...
public abstract class EntityHanging extends Entity implements ITrack {
    ...
    public void tick() {
        ...
        if (this.e++ == this.world.spigotConfig.hangingTickFrequency && !this.world.isStatic) { // Spigot - 100 -> this.world.spigotConfig.hangingTickFrequency
            ...
            if (!this.dead && !this.survives()) {
                ...
                HangingBreakEvent.RemoveCause cause = (!material.equals(Material.AIR) ? HangingBreakEvent.RemoveCause.OBSTRUCTION : HangingBreakEvent.RemoveCause.PHYSICS); // TODO: This feels insufficient to catch 100% of suffocation cases
    ...
    public void a(NBTTagCompound nbttagcompound) {
        if (nbttagcompound.hasKeyOfType("Direction", 99)) {
        	...
        	this.direction = switch (nbttagcompound.getByte("Dir")) {
	            case 0 -> 2;
	            case 1 -> 1;
	            case 2 -> 0;
	            case 3 -> 3;
				default -> throw new IllegalArgumentException("Unexpected value: " + nbttagcompound.getByte("Dir")); // Rinny
            };
        }
    ...
    public void track(EntityTracker tracker) {
    	tracker.addEntity(this, 160, Integer.MAX_VALUE);
    }
}
