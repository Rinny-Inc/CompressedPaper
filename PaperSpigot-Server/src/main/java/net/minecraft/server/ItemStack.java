...
public final class ItemStack {
    ...
    public boolean placeItem(EntityHuman entityhuman, World world, int i, int j, int k, int l, float f, float f1, float f2) {
        ...
        if (flag && world.captureTreeGeneration && !world.capturedBlockStates.isEmpty()) { // Rinny - use !isEmpty instead of size > 0
        ...
        if (flag) {
            ...
            // Rinny start
            if (!blocks.isEmpty()) {
            	placeEvent = blocks.size() > 1 ? CraftEventFactory.callBlockMultiPlaceEvent(world, entityhuman, blocks, i, j, k) : CraftEventFactory.callBlockPlaceEvent(world, entityhuman, blocks.get(0), i, j, k);
            }
            /*if (blocks.size() > 1) {
            ...
            if (placeEvent != null && (placeEvent.isCancelled() || !placeEvent.canBuild())) {
                ...
                for (BlockState blockstate : blocks) {
                    ...
                    if (this.getItem() instanceof ItemBlock itemBlock) { // Rinny
        ...
        if (!(entityliving instanceof EntityHuman) || !((EntityHuman) entityliving).abilities.canInstantlyBuild) {
            if (this.g()) {
                if (this.isDamaged(i, entityliving.aI(), entityliving)) { // Spigot
                    entityliving.a(this);
                    --this.count;
                    if (entityliving instanceof EntityHuman entityhuman) { // Rinny
