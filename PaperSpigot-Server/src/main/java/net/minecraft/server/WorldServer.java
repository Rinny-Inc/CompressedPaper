...
public class WorldServer extends World {
    ...
    public TileEntity getTileEntity(int i, int j, int k) {
        ...
        final Block type = getType(i, j, k);
    ...
    public void doTick() {
        ...
        if (this.getGameRules().getBoolean("doMobSpawning") && (this.allowMonsters || this.allowAnimals) && (this instanceof WorldServer && !this.players.isEmpty())) { // Rinny - use !isEmpty instead of size > 0
    ...
    protected void g() {
        ...
        for (net.minecraft.util.gnu.trove.iterator.TLongShortIterator iter = chunkTickList.iterator(); iter.hasNext();) {
            iter.advance();
            chunkCoord = iter.key();
            chunkX = World.keyToX(chunkCoord);
            chunkZ = World.keyToZ(chunkCoord);
    ...
    protected void b(WorldSettings worldsettings) {
        if (!this.worldProvider.e()) {
            this.worldData.setSpawn(0, this.worldProvider.getSeaLevel(), 0, 0.0f, 90.0f);
        } else {
            this.isLoading = true;
            ...
            this.worldData.setSpawn(i, j, k, 0.0f, 90.0f);
            this.isLoading = false;
            if (worldsettings.c()) {
                this.k();
            }
    ...
    public void save(boolean flag, IProgressUpdate iprogressupdate) throws ExceptionWorldConflict { // CraftBukkit - added throws
        if (this.chunkProvider.canSave()) {
            ...
            // TODO: PandaSpigot removed it because as it seems Bukkit has its own system for this.
    ...
    protected void a(Entity entity) {
        ...
        if (aentity != null) {
            for (final Entity entities : aentity) {
                this.entitiesById.a(entities.getId(), entities);
            }
        }
    }
    ...
    protected void b(Entity entity) {
        ...
        if (aentity != null) {
        	for (final Entity entities : aentity) {
                this.entitiesById.d(entities.getId());
            }
        }
    }
