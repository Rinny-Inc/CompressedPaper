...
public class EntityWither extends EntityMonster implements IRangedEntity, ITrack {
    ...
    public EntityWither(World world) {
        ...
        this.targetSelector.a(2,
                new PathfinderGoalNearestAttackableTarget(this, EntityInsentient.class, 0, false, false, bw));
        this.b = 50;
    }
    ...
    public void e() {
        ...
        for (j = 0; j < 3; ++j) {
            ...
            this.world.addParticle("smoke", d8 + this.random.nextGaussian() * 0.30000001192092896D,
                    d9 + this.random.nextGaussian() * 0.30000001192092896D,
                    d10 + this.random.nextGaussian() * 0.30000001192092896D, 0.0D, 0.0D, 0.0D);
            if (flag && this.world.random.nextInt(4) == 0) {
                this.world.addParticle("mobSpell", d8 + this.random.nextGaussian() * 0.30000001192092896D,
                        d9 + this.random.nextGaussian() * 0.30000001192092896D,
    ...
    protected void bn() {
        ...
        if (this.ca() > 0) {
            ...
            if (i <= 0) {
                ...
                for (EntityHuman hplayer : this.world.players) {
                    final EntityPlayer player = (EntityPlayer) hplayer;
                    ...
                    if (distanceSquared > viewDistance * viewDistance) {
                        ...
                        player.playerConnection.sendPacket(new PacketPlayOutWorldEvent(1013, (int) relativeX,
                                (int) this.locY, (int) relativeZ, 0, true));
                    } else {
                        player.playerConnection.sendPacket(new PacketPlayOutWorldEvent(1013, (int) this.locX,
                                (int) this.locY, (int) this.locZ, 0, true));
                    }
                }
                // CraftBukkit end
            }
            ...
            for (i = 1; i < 3; ++i) {
                if (this.ticksLived >= this.bt[i - 1]) {
                    ...
                    if (this.world.difficulty == EnumDifficulty.NORMAL
                            || this.world.difficulty == EnumDifficulty.HARD) {
                        int i1001 = i - 1;
                    ...
                    if (j > 0) {
                        ...
                        if (entity != null && entity.isAlive() && this.f(entity) <= 900.0D
                        ...
                        for (int l1 = -1; l1 <= 1; ++l1) {
                            for (int i2 = 0; i2 <= 3; ++i2) {
                                ...
                                if (block.getMaterial() != Material.AIR && block != Blocks.BEDROCK
                                        && block != Blocks.ENDER_PORTAL && block != Blocks.ENDER_PORTAL_FRAME
    ...
    private void a(int i, EntityLiving entityliving) {
        this.a(i, entityliving.locX, entityliving.locY + (double) entityliving.getHeadHeight() * 0.5D,
