...
public class PacketPlayOutPlayerInfo extends Packet {
    ...
    */
    ...
    private String limitedUsername;
    public void b(PacketDataSerializer packetdataserializer) throws IOException {
        String username = this.username;
        
        if (username != null && username.length() > 16) {
            if (limitedUsername == null) {
            	limitedUsername = this.username.substring(0, 16);
            }
    ...
    /*public void b(PacketDataSerializer packetdataserializer) throws IOException {
        ...
        // Rinny start
        if (username != null && packetdataserializer.version < 28 && username.length() > 16) {
            if (limitedUsername == null) {
            	limitedUsername = ValidateUtils.limit(this.username, 16);
            }
            username = limitedUsername;
        }
        // Rinny stop
        ...
        if ( packetdataserializer.version >= 20 )
        {
            ...
            switch ( action )
            {
                case ADD_PLAYER:
                    ...
                    final PropertyMap properties = player.getProperties();
    ...
    public void write47(PacketDataSerializer packetdataserializer) throws IOException {
    	String username = this.username;
        if (action == PlayerInfo.ADD_PLAYER && username != null && username.equals(player.getName())) {
            username = null;
        }
        packetdataserializer.b( action.getValue() );
        packetdataserializer.b( 1 );
