...
public class PacketDataSerializer extends ByteBuf {
    ...
    private static final String[] POTION_NAMES_107 = new String[] { 
    	      "mundane", "regeneration", "swiftness", "fire_resistance", "poison", "healing", "night_vision", "empty", "weakness", "strength", 
    	      "slowness", "leaping", "harming", "water_breathing", "invisibility", "empty" };
    ...
    public void a(ItemStack itemstack) {
        if (itemstack == null || itemstack.getItem() == null) { // CraftBukkit - NPE fix itemstack.getItem()
            ...
            // Spigot start - protocol patch
            if (this.version >= 47)
            	id = SpigotDebreakifier.getItemId(Item.getId(itemstack.getItem())); 
            if (this.version >= 107 && itemstack.getItem() == Items.POTION) {
            	id = ((itemstack.getData() & 0x4000) == 0) ? 373 : 438;
            	data = 0;
            } 
            // Spigot end
            this.writeShort(id);
            ...
            if (itemstack.getItem() == Items.POTION && this.version >= 107) {
                String name;
                nbttagcompound = (nbttagcompound != null) ? (NBTTagCompound)nbttagcompound.clone() : new NBTTagCompound();
                if ((itemstack.getData() & 0x3F) == 0) {
                  name = "water";
                } else if ((itemstack.getData() & 0x3F) == 16) {
                  name = "awkward";
                } else {
                  name = POTION_NAMES_107[itemstack.getData() & 0xF];
                  if ((itemstack.getData() & 0x20) != 0) {
                    name = "strong_" + name;
                  } else if ((itemstack.getData() & 0x40) != 0) {
                    name = "long_" + name;
                  } 
                } 
                nbttagcompound.set("Potion", new NBTTagString("minecraft:" + name));
            } 
            // Spigot end
    ...
    public String c(int i) throws IOException { // CraftBukkit - throws IOException
    	...
    	int j = this.a();
    	if (j < 0 || j > i * 4) {
    	    throw new IOException("The received encoded string buffer length is invalid. Got: " + j);
    	}
