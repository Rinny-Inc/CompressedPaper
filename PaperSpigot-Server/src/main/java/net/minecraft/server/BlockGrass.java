    ...
    public void a(World world, int i, int j, int k, Random random) {
        if (!world.isStatic) {
            if (world.getLightLevel(i, j + 1, k) < 4 && world.getType(i, j + 1, k).k() > 2) {
                ...
                org.bukkit.World bworld = world.getWorld(); // Rinny
                for (int l = 0; l < numGrowth; ++l) { // Spigot
                    ...
                    if (world.getType(i1, j1, k1) == Blocks.DIRT && world.getData(i1, j1, k1) == 0 && block.k() <= 2 && world.getLightLevel(i1, j1 + 1, k1) >= 4) { // PandaSpigot - Move last check before isLightLevel to avoid unneeded light checks
                        // CraftBukkit start
                    	//org.bukkit.World bworld = world.getWorld(); // Rinny - moved up out of the for loop
