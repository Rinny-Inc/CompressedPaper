...
public abstract class EntityHuman extends EntityLiving implements ICommandListener {
    ...
    public boolean shieldBlocking = false; // 1.9
    ...
    public EntityHuman(World world, GameProfile gameprofile) {
        ...
        this.setPositionRotation((double) chunkcoordinates.x + 0.5D, (double) (chunkcoordinates.y + 1), (double) chunkcoordinates.z + 0.5D, world.getWorldData().spawnYaw(), world.getWorldData().spawnPitch());
        this.aZ = 180.0F;
    ...
    protected void c() {
        super.c();
        this.datawatcher.a( 16, Byte.valueOf((byte)0)); // Spigot - protocol patch, handle metadata usage change (show cape -> collisions)
    ...
    public void bB() {
    	if (this.f != null) {
    		...
    		if (player.playerConnection.networkManager.getVersion() >= 107 && this.f.getItem().d(this.f) == EnumAnimation.BLOCK) {
    			player.playerConnection.sendPacket(new PacketPlayOutSetSlot(0, 45, null));
    ...
    public boolean isEating() { // Rinny - add isEating
        return this.by() && this.f.getItem().d(this.f) == EnumAnimation.EAT;
    }
    ...
    public void tick() {
        if (this.f != null) {
            ...
            if (itemstack == this.f && isRelevantItem(itemstack)) { // Rinny - add specific items because we don't want this to be triggered when not needed
                if (this.g <= 25 && (this.g & 3) == 0) { // Rinny - use bitwise
                    this.c(itemstack, 5);
                }
                ...
                if (((EntityPlayer)this).playerConnection.networkManager.getVersion() >= 107) {
                	if (this.f != null && this.f.getItem().d(this.f) == EnumAnimation.BLOCK && this.g < 71980 && !this.shieldBlocking) {
                		this.bA();
                	}
                }
            } else {
        ...
        if (this.isSleeping()) {
            ...
            if (!this.world.isStatic) {
            	...
            	this.a(!this.j(), true, this.j() && this.world.w()); // Rinny // TODO: check if does shit
            }
        ...
        final double d1 = this.locY - this.by;
        final double d2 = this.locZ - this.bz;
    ...
    private static boolean isRelevantItem(ItemStack itemstack) { // Rinny
        return itemstack.getItem() instanceof ItemFood || itemstack.getItem() instanceof ItemBow || itemstack.getItem() instanceof ItemSword || itemstack.getItem() instanceof ItemPotion;
    }
    ...
    protected void c(ItemStack itemstack, int i) {
    	// Rinny - don't execute check for nothing
    	switch (itemstack.o()) {
    		case DRINK: {
    		...
    		case EAT: {
    			float f1 = -this.pitch * 3.1415927F / 180.0F; // Rinny - cache
    			float f2 = -this.yaw * 3.1415927F / 180.0F; // Rinny - cache
    			String s = "iconcrack_" + Item.getId(itemstack.getItem()) + (itemstack.usesData() ? "_" + itemstack.getData() : ""); // Rinny - cache
    			for (int j = 0; j < i; ++j) {
    ...
    protected void p() {
        if (this.f != null) {
            ...
            if (event.isCancelled()) {
                // Update client
                if (this instanceof EntityPlayer ep) {
                    ep.playerConnection.sendPacket(new PacketPlayOutSetSlot((byte) 0, activeContainer.getSlot((IInventory) this.inventory, this.inventory.itemInHandIndex).index, this.f));
                    // Spigot Start
    ...
    public void e() {
        ...
        this.aQ = (!this.isSprinting() ? this.bJ : (float) ((double) this.aQ + (double) this.bJ * 0.3D));
        this.i((float) attributeinstance.getValue());
        float f = Math.min((float)Math.sqrt(this.motX * this.motX + this.motZ * this.motZ), 0.1F);
        ...
        if (this.getHealth() > 0.0F) {
            AxisAlignedBB axisalignedbb = (this.vehicle != null && !this.vehicle.dead ? this.boundingBox.a(this.vehicle.boundingBox).grow(1.0D, 0.0D, 1.0D) : this.boundingBox.grow(1.0D, 0.5D, 1.0D));
    ...
    public void die(DamageSource damagesource) {
        ...
        if (damagesource != null) {
        	final float compute = (this.az + this.yaw) * 3.1415927F / 180.0F;
            this.motX = (double) (-MathHelper.cos(compute) * 0.1F);
            this.motZ = (double) (-MathHelper.sin(compute) * 0.1F);
        } else {
    ...
    public EntityItem a(ItemStack itemstack, boolean flag, boolean flag1) {
        if (itemstack == null) {
            ...
            if (flag) {
                ...
                f = 0.3F;
                final float precomputed_yaw = this.yaw / 180.0F * 3.1415927F; // Rinny - compute it once
                final float precomputed_pitch = this.pitch / 180.0F * 3.1415927F; // Rinny - compute it once
                entityitem.motX = (double) (-MathHelper.sin(precomputed_yaw) * MathHelper.cos(precomputed_pitch) * f);
                entityitem.motZ = (double) (MathHelper.cos(precomputed_yaw) * MathHelper.cos(precomputed_pitch) * f);
                entityitem.motY = (double) (-MathHelper.sin(precomputed_pitch) * f + 0.1F);
                f = 0.02F;
    ...
    public float a(Block block, boolean flag) {
        ...
        if (f > 1.0F) {
            ...
            if (i > 0 && itemstack != null) {
                ...
                f += f1 * (!itemstack.b(block) && f <= 1.0F ? 0.08F : 1.0F);
            }
        }

        MobEffectList mel;
        if (this.hasEffect((mel = MobEffectList.FASTER_DIG))) {
            f *= 1.0F + (float) (this.getEffect(mel).getAmplifier() + 1) * 0.2F;
        }

        if (this.hasEffect((mel = MobEffectList.SLOWER_DIG))) {
            f *= 1.0F - (float) (this.getEffect(mel).getAmplifier() + 1) * 0.2F;
        }
    ...
    public boolean damageEntity(DamageSource damagesource, float f) {
        ...
        if (damagesource.r()) {
        	switch (this.world.difficulty) { // Rinny
	        	case PEACEFUL -> {
	        		return false;
	        	}
	        	case EASY -> f = f / 2.0F + 1.0F;
	        	case HARD -> f = f * 3.0F / 2.0F;
        	}
        }
        ...
        if (entity instanceof EntityArrow ea && ea.shooter != null) {
            entity = ea.shooter;
        }
    ...
    public boolean a(EntityHuman entityhuman) {
        ...
        if (entityhuman instanceof EntityPlayer thatPlayer) { // Rinny
    ...
    public void attack(Entity entity) {
        ...
        if (entity.av()) {
            if (!entity.j(this)) {
                ...
                byte i = (byte) Boolean.compare(this.hasExtraKnockback(), false); // Rinny - replace int to byte
                float f1 = 0.0F;
                ...
                if (f > 0.0F || f1 > 0.0F) {
                	final boolean crit = this.isCriticalHit(f, entity);

                    if (crit/* && f > 0.0F*/) { // Rinny - removed double code
                        f *= 1.5F;
                    }
                    ...
                    final double[] victimMotion = {entity.motX, entity.motY, entity.motZ};
                    final boolean flag2 = entity.damageEntity(DamageSource.playerAttack(this), f);

                    if (flag2) {
                        if (i > 0) {
                        	final float yawRadians = this.yaw * 3.1415927F / 180.0F;
                        	final float sinYaw = -MathHelper.sin(yawRadians);
                        	final float cosYaw = MathHelper.cos(yawRadians);
                        	final float horizontalKnockback = (float) (i * world.paperSpigotConfig.knockbackSprintHorizontal);
                        	final double knockbackX = sinYaw * horizontalKnockback;
                        	final double knockbackZ = cosYaw * horizontalKnockback;
                        	
                        	entity.g(knockbackX, world.paperSpigotConfig.knockbackSprintVertical, knockbackZ);
                        	
                            this.motX *= 0.6D;
                            this.motZ *= 0.6D;
                            this.setExtraKnockback(false); // Nacho - fix sprint desync
                        }
                        if (entity instanceof EntityPlayer victim && entity.velocityChanged) { // Rinny
                            final org.bukkit.util.Vector velocity = victim.getBukkitEntity().getVelocity();

                            final PlayerVelocityEvent event = new PlayerVelocityEvent(victim.getBukkitEntity(), velocity);
                            this.world.getServer().getPluginManager().callEvent(event);

                            if (!event.isCancelled()) {
                                if (!velocity.equals(event.getVelocity())) {
                                	victim.getBukkitEntity().setVelocityDirect(velocity);
                                }
                                victim.playerConnection.sendPacket(new PacketPlayOutEntityVelocity(entity));
                            }
                        	victim.velocityChanged = false;
                        	victim.motX = victimMotion[0];
                        	victim.motY = victimMotion[1];
                        	victim.motZ = victimMotion[2];
                        }
                        ...
                        if (entity instanceof EntityComplexPart ecp) {
                        	final IComplex icomplex = ecp.owner;
                        ...
                        if (itemstack != null && object instanceof EntityLiving el) {
                            itemstack.a(el, this);
                            // CraftBukkit - bypass infinite items; <= 0 -> == 0
                            if (itemstack.count == 0) {
                                this.bG();
                        ...
                        if (entity instanceof EntityLiving) {
                            ...
                            if (j > 0) {
                                // CraftBukkit start - Call a combust event when somebody hits with a fire enchanted item
                            	final EntityCombustByEntityEvent combustEvent = new EntityCombustByEntityEvent(this.getBukkitEntity(), entity.getBukkitEntity(), j * 4);
                                org.bukkit.Bukkit.getPluginManager().callEvent(combustEvent);
    ...
    private boolean isCriticalHit(float f, Entity entity) {
        return !world.paperSpigotConfig.disablePlayerCrits && this.fallDistance > 0.0F && !this.onGround && !this.h_() && !this.M() && !this.hasEffect(MobEffectList.BLINDNESS) && this.vehicle == null && entity instanceof EntityLiving;
    }
    ...
    public EnumBedResult a(int i, int j, int k) {
        if (!this.world.isStatic) {
            ...
            double d0 = 8.0D;
            double d1 = 5.0D;
            List<Entity> list = this.world.a(EntityMonster.class, AxisAlignedBB.a((double) i - d0, (double) j - d1, (double) k - d0, (double) i + d0, (double) j + d1, (double) k + d0));
        ...
        if (this.getBukkitEntity() instanceof Player player) { // Rinny
        ...
        if (this.world.isLoaded(i, j, k)) {
            ...
            final int i1 = BlockBed.l(l);
            final float f = switch (i1) {
				case 1 -> 0.1F;
				case 3 -> 0.9F;
				default -> 0.5F;
			};
			final float f1 = switch (i1) {
				case 0 -> 0.9F;
				case 2 -> 0.1F;
				default -> 0.5F;
			};
    ...
    public void a(boolean flag, boolean flag1, boolean flag2) {
        ...
        if (this.getBukkitEntity() instanceof Player player) { // Rinny
            org.bukkit.block.Block bed = (chunkcoordinates != null ? this.world.getWorld().getBlockAt(chunkcoordinates.x, chunkcoordinates.y, chunkcoordinates.z) : this.world.getWorld().getBlockAt(player.getLocation()));
        ...
        if (flag2) this.setRespawnPosition(this.bB, false);
    }
	...
	protected void b(int i, boolean flag) {
		...
		if (flag) {
			...
			this.datawatcher.watch(16, Byte.valueOf((byte)(b0 & (1 << i ^ 0xFFFFFFFF))));
		} 
	}
    // Spigot end
    ...
    public void bj() {
        ...
        this.applyExhaustion((this.isSprinting() ? world.spigotConfig.sprintExhaustion : world.spigotConfig.walkExhaustion));
    }
        ...
        if (this.vehicle == null) {
            ...
            if (this.a(Material.WATER)) {
                ...
                if (i > 0) {
                    ...
                    this.applyExhaustion((this.isSprinting() ? 0.099999994F : 0.01F) * (float) i * 0.01F);
                }
            } else {
                i = Math.round((float)Math.sqrt(d0 * d0 + d2 * d2) * 100.0F);
                if (i > 25) {
    ...
    public void a(ItemStack itemstack, int i) {
        if (itemstack != this.f) {
        	if (itemstack.getItem().d(itemstack) == EnumAnimation.BLOCK && !isBlocking()) {
        		this.shieldBlocking = false;
                EntityPlayer player = (EntityPlayer)this;
                if (player.playerConnection.networkManager.getVersion() >= 107) {
                	player.playerConnection.sendPacket(new PacketPlayOutSetSlot(0, 45, itemstack)); 
                }
        	} 
            this.f = itemstack;
            this.g = i;
    ...
    public void setAbsorptionHearts(float f) {
        this.getDataWatcher().watch(17, Float.valueOf(Math.max(f, 0.0F)));
    }
