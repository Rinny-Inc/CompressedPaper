...
public class LoginListener implements PacketLoginInListener {
	...
	// PaperSpigot end
    //private static final ExecutorService POOL = Executors.newCachedThreadPool(new ThreadFactoryBuilder().setNameFormat("User Authenticator #%d").build());
    ...
    public void initUUID() {
        final UUID uuid = (networkManager.spoofedUUID != null ? networkManager.spoofedUUID : UUID.nameUUIDFromBytes(("OfflinePlayer:" + this.i.getName()).getBytes(Charsets.UTF_8)));
    ...
    public void c() {
        ...
        */
        ...
        //if (s == null) {
            ...
            // CraftBukkit end
        //} else {
        if (s != null) { // replace else by if
            this.g = EnumProtocolState.e;
            // Spigot start
            if ( networkManager.getVersion() >= 27 ) {
            	this.networkManager.handle(new org.spigotmc.ProtocolInjector.PacketLoginCompression(256), future -> networkManager.enableCompression());
            ...
            this.server.getPlayerList().handlePlayerLogin(this.networkManager, this.server.getPlayerList().processLogin(this.i, s)); // CraftBukkit - add player reference
        }
    }

    public void a(IChatBaseComponent ichatbasecomponent) {
    	c.info("{} lost connection: {}", this.getName(), ichatbasecomponent.c());
    }
    ...
    public String getName() {
    	...
    	final String socketAddress = networkManager == null ? null : (networkManager.getSocketAddress() == null ? null : networkManager.getSocketAddress().toString());
    	return this.i != null ? this.i.toString() + " (" + socketAddress + ")" : socketAddress;
    }

    public void a(EnumProtocol enumprotocol, EnumProtocol enumprotocol1) {
        Validate.validState(this.g == EnumProtocolState.e || this.g == EnumProtocolState.HELLO, "Unexpected change in protocol");
        Validate.validState(enumprotocol1 == EnumProtocol.PLAY || enumprotocol1 == EnumProtocol.LOGIN, "Unexpected protocol " + enumprotocol1);
    }
    ...
    public void a(PacketLoginInStart packetlogininstart) {
        ...
        if (this.server.getOnlineMode() && !this.networkManager.c()) {
            ...
            this.networkManager.handle(new PacketLoginOutEncryptionBegin(this.j, this.server.K().getPublic(), this.e));
        } else {
