    ...
    public int spawnEntities(WorldServer worldserver, boolean flag, boolean flag1, boolean flag2) {
        if (!flag && !flag1) {
            ...
            for (int j1 = 0; j1 < j; ++j1) {
                ...
                final int limit = switch (enumcreaturetype) {
                    case MONSTER -> worldserver.getWorld().getMonsterSpawnLimit();
                    case CREATURE -> worldserver.getWorld().getAnimalSpawnLimit();
                    case WATER_CREATURE -> worldserver.getWorld().getWaterAnimalSpawnLimit();
                    case AMBIENT -> worldserver.getWorld().getAmbientSpawnLimit();
