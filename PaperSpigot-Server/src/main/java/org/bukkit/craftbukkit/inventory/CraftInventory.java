...
public class CraftInventory implements Inventory {
    ...
    private int firstPartialIndex(ItemStack item, int fromIndex) {
        ItemStack[] inventory = getContents();
        ItemStack filteredItem = CraftItemStack.asCraftCopy(item);
        if (filteredItem == null) return -1;

        for (int i = fromIndex; i < inventory.length; i++) {
            ItemStack cItem = inventory[i];
            if (cItem != null && cItem.getAmount() < cItem.getMaxStackSize() && cItem.isSimilar(filteredItem)) {
                return i;
            }
    ...
    public HashMap<Integer, ItemStack> addItem(ItemStack... items) {
        ...
        Map<String, Integer> lastPartialIndex = new HashMap<>();

        for (int i = 0; i < items.length; i++) {
            ItemStack item = items[i];
            String key = item.getType().name() + ":" + item.getDurability(); // Use durability for exact match

            while (true) {
                int fromIndex = lastPartialIndex.getOrDefault(key, 0);
                int firstPartial = firstPartialIndex(item, fromIndex);

                if (firstPartial == -1) {
                    // Cache empty slot only once per stack attempt
                    int firstFree = firstEmpty();
                    ...
                    lastPartialIndex.put(key, firstPartial + 1); // Move start index forward
    ...
    public HashMap<Integer, ItemStack> removeItem(ItemStack... items) {
        ...
        for (int i = 0; i < items.length; i++) {
            ItemStack item = CraftItemStack.asCraftCopy(items[i]); // OLD => items[i];
    ...
    public InventoryType getType() {
        ...
        if (inventory instanceof InventoryCrafting ic) {
            return ic.getInvType(); // Rinny
        ...
        } else if (inventory instanceof CraftInventoryCustom.MinecraftInventory mi) {
            return mi.getType();
