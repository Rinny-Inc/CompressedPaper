...
public final class CraftItemStack extends ItemStack {
    ...
    public static ItemMeta getItemMeta(net.minecraft.server.ItemStack item) {
        ...
        return switch (getType(item)) {
            case WRITTEN_BOOK,
            	 BOOK_AND_QUILL -> new CraftMetaBook(item.tag);
            case SKULL_ITEM -> new CraftMetaSkull(item.tag);
            case LEATHER_HELMET,
            	 LEATHER_CHESTPLATE,
            	 LEATHER_LEGGINGS,
            	 LEATHER_BOOTS -> new CraftMetaLeatherArmor(item.tag);
            case POTION -> new CraftMetaPotion(item.tag);
            case MAP -> new CraftMetaMap(item.tag);
            case FIREWORK -> new CraftMetaFirework(item.tag);
            case FIREWORK_CHARGE -> new CraftMetaCharge(item.tag);
            case ENCHANTED_BOOK -> new CraftMetaEnchantedBook(item.tag);
            default -> new CraftMetaItem(item.tag);
        };
    }
