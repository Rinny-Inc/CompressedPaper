...
public final class AsynchronousExecutor<P, T, C, E extends Throwable> {
    ...
    class Task implements Runnable {
        ...
        void finish() throws E {
            if (state < STAGE_1_COMPLETE) {
                throw new IllegalStateException("Task not ready for finishing");
            }
    ...
    final Map<P, Task> tasks = new ConcurrentHashMap<>();//new HashMap<P, Task>();
    ...
    public void add(P parameter, C callback) {
    	...
    	Task task = tasks.computeIfAbsent(parameter, p -> {
