...
public class CachedMojangAPIConnection extends HttpURLConnection
{
    ...
    public InputStream getInputStream() throws IOException
    {
        ...
        {
            outClosed = true;
            JsonArray users = JsonParser.parseString( new String( outputStream.toByteArray(), Charsets.UTF_8 ) ).getAsJsonArray(); // Rinny - replaced new JsonParser -> JsonParser.parseString
            ...
            if ( missingUsers.length() > 2 )
            {
                ...
                out.flush();
                out.close();
                JsonArray newUsers = JsonParser.parseReader( new InputStreamReader( connection.getInputStream(), Charsets.UTF_8 ) ).getAsJsonArray(); // Rinny - replaced new JsonParser -> JsonParser.parseReader
