...
public class ActivationRange
{
    ...
    public static void activateEntities(World world)
    {
        ...
        for (Entity player : world.players)
        {
            ...
            final int i = MathHelper.floor( maxBB.a / 16.0D );
            final int j = MathHelper.floor( maxBB.d / 16.0D );
            final int k = MathHelper.floor( maxBB.c / 16.0D );
            final int l = MathHelper.floor( maxBB.f / 16.0D );
            ...
            for ( int i1 = i; i1 <= j; ++i1 )
            {
                ...
                {
                	// Rinny
                	final Chunk chunk = world.getChunkIfLoaded(i1, j1); // getChunkAt
                	if (chunk != null) {
    ...
    public static boolean checkEntityImmunities(Entity entity)
    {
        ...
        // special cases.
        if ( entity instanceof EntityLiving living) { // Rinny
            if ( living.attackTicks > 0 || living.hurtTicks > 0 || !living.effects.isEmpty() ) // Rinny - use !isEmpty instead of size > 0
            {
                return true;
            }
            if ( entity instanceof EntityCreature creature && creature.target != null )
            {
                return true;
            }
            if ( entity instanceof EntityVillager villager && villager.bY() /* Getter for first boolean */ )
            {
                return true;
            }
            if ( entity instanceof EntityAnimal animal) { // Rinny
