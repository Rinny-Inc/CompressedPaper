...
public enum Effect {
    ...
    private static final ImmutableMap<Integer, Effect> BY_ID;
    private static final ImmutableMap<String, Effect> BY_NAME;
    ...
    static {
    	ImmutableMap.Builder<Integer, Effect> byIdBuilder = ImmutableMap.builder();
        for (Effect effect : values()) {
            if (effect.type != Type.PARTICLE) {
                byIdBuilder.put(effect.id, effect);
            }
        }
    ...
    static {
    	ImmutableMap.Builder<String, Effect> byNameBuilder = ImmutableMap.builder();
        for (Effect effect : values()) {
            if (effect.type == Type.PARTICLE) {
                byNameBuilder.put(effect.particleName, effect);
            }
        }
