...
public enum EntityType {
    ...
    /*HOLOGRAM(null, Hologram.class, -1, false),*/
    /**
    ...
    static {
    	ImmutableMap.Builder<String, EntityType> nameBuilder = ImmutableMap.builder();
        ImmutableMap.Builder<Short, EntityType> idBuilder = ImmutableMap.builder();

        for (EntityType type : EnumSet.allOf(EntityType.class)) {
        	if (type.name != null) {
        		nameBuilder.put(type.name.toLowerCase(), type);
        	}
        	if (type.typeId > 0) {
        		idBuilder.put(type.typeId, type);
        	}
        }

        NAME_MAP = nameBuilder.build();
        ID_MAP = idBuilder.build();
