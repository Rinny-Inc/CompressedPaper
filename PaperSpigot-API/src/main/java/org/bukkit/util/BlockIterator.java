...
public class BlockIterator implements Iterator<Block> {
    ...
    public BlockIterator(World world, Vector start, Vector direction, double yOffset, int maxDistance) {
        ...
        final Block startBlock = this.world.getBlockAt((int)Math.floor(startClone.getX()), (int)Math.floor(startClone.getY()), (int)Math.floor(startClone.getZ()));
    ...
    private void scan() {
        ...
        if (secondError > 0 && thirdError > 0) {
            ...
            blockQueue[1] = blockQueue[2].getRelative(((long) secondStep) * ((long) thirdError) < ((long) thirdStep) * ((long) secondError) ? secondFace : thirdFace);
            blockQueue[0] = blockQueue[1].getRelative(((long) secondStep) * ((long) thirdError) < ((long) thirdStep) * ((long) secondError) ? thirdFace : secondFace);
            thirdError -= gridSize;
            secondError -= gridSize;
            currentBlock = 2;
            return;
